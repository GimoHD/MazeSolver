...
public class GenerateMaze {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter an integer to determine the size of the generateMaze");

        String entered = scanner.nextLine();
        int size = Integer.parseInt(entered);

        System.out.println("Generating generateMaze...");
        GenerateMaze generateMaze = new GenerateMaze(size);
        generateMaze.print();

        List<String> lines = generateMaze.toListOfStrings();
    ...
    private void addSurroundingWalls(int row, int col) {
        if (row > 1 && grid[row - 2][col] == unvisited && grid[row - 1][col] == wall)
            listOfWalls.add(new Coordinate(row - 1, col));
        if (row < gridSize - 2 && grid[row + 2][col] == unvisited && grid[row + 1][col] == wall)
            listOfWalls.add(new Coordinate(row + 1, col));
        if (col > 1 && grid[row][col - 2] == unvisited && grid[row][col - 1] == wall)
            listOfWalls.add(new Coordinate(row, col - 1));
        if (col < gridSize - 2 && grid[row][col + 2] == unvisited && grid[row][col + 1] == wall)
            listOfWalls.add(new Coordinate(row, col + 1));
    }
